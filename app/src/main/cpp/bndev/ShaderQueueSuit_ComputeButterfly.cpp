...
void ShaderQueueSuit_ComputeButterfly::create_uniform_buffer(VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
	bufferByteCount = sizeof(int) * 1;
...
void ShaderQueueSuit_ComputeButterfly::destroy_uniform_buffer(VkDevice& device)
{
	vk::vkDestroyBuffer(device, uniformBuf, NULL);
	vk::vkFreeMemory(device, memUniformBuf, NULL);
}

void ShaderQueueSuit_ComputeButterfly::create_pipeline_layout(VkDevice& device)
{
	...
	push_constant_ranges[0].size = sizeof(int) * 3;
...
void ShaderQueueSuit_ComputeButterfly::destroy_pipeline_layout(VkDevice& device)
{
	vk::vkDestroyPipelineLayout(device, pipelineLayout, NULL);
}


void ShaderQueueSuit_ComputeButterfly::init_descriptor_set(VkDevice& device)
{
...
void ShaderQueueSuit_ComputeButterfly::create_shader(VkDevice& device)
{
	std::string compStr = FileUtil::loadAssetStr("shader/Butterfly.comp");//加载计算着色器脚本
...
void ShaderQueueSuit_ComputeButterfly::destroy_shader(VkDevice& device)
{
	vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
}

void ShaderQueueSuit_ComputeButterfly::create_pipe_line(VkDevice& device)
{
...
void ShaderQueueSuit_ComputeButterfly::destroy_pipe_line(VkDevice& device)
{
...
ShaderQueueSuit_ComputeButterfly::ShaderQueueSuit_ComputeButterfly(VkDevice* deviceIn, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_ComputeButterfly::~ShaderQueueSuit_ComputeButterfly()
{
