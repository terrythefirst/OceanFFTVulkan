...
void ShaderQueueSuit_ComputeTwiddleFactors::createBit_reversedDataBuffer(int dataByteCount, VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
	...
	buf_info.flags = 0;
	VkResult result = vk::vkCreateBuffer(device, &buf_info, NULL, &bit_reversedBufCompute);
	...
	bit_reversedBufferInfoCompute.buffer = bit_reversedBufCompute;
	bit_reversedBufferInfoCompute.offset = 0;
	bit_reversedBufferInfoCompute.range = mem_reqs.size;
}
...
void ShaderQueueSuit_ComputeTwiddleFactors::initBitReversedIndices(int n) {
	bitReversedIndices = (int*)malloc(sizeof(int)*n);
...
{
	std::string compStr = FileUtil::loadAssetStr("shader/twiddleFactors.comp");//加载计算着色器脚本
